...
app.get("/", async (req, res) => {
  ...
  const isBot = /(Googlebot|Slackbot|bingbot|facebookexternalhit|CodaBot|Preview)/i.test(
    ua
  );

  // Get real user identity from headers
  const codaUserEmail = req.headers["x-coda-user-email"] || "Unknown";
  const codaUserName = req.headers["x-coda-user-name"] || "Unknown";
    ...
    rows: [
      {
        cells: [
          ...
          { column: "c-Bnd91_0ohs", value: codaUserEmail },
          { column: "c-uOLRfDdGlm", value: codaUserName },
  ...
  try {
    const codaRes = await fetch(
      `https://coda.io/apis/v1/docs/${CODA_DOC_ID}/tables/${encodeURIComponent(
        CODA_TABLE_ID
      )}/rows`,
      {
        method: "POST",
